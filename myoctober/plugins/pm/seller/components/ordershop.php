...
class ordershop extends ComponentBase
{
    ...
    {
        $this->orders = CheckoutDetails::where('id_seller',Session::get('shopid'))->join('pm_product_checkout','pm_product_checkout_details.id_checkout','=','pm_product_checkout.id')->groupby('id_checkout')->get();
    ...
    public function onConfirm(){
        $a = CheckoutDetails::where('id_seller',Session::get('shopid'))->where('id_checkout',Input::get('id'))->get();
        for($i=0;$i<count($a);$i++){
